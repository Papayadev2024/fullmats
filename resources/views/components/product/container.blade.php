<div x-data="{ showAmbiente: false }" @mouseenter="showAmbiente = true" @mouseleave="showAmbiente = false"
    ...
    <div class="absolute top-2 left-2">
      @if ($item->tags)
        @foreach ($item->tags as $tag)
          <div class="px-4 mb-1">
            <span
              class="block font-semibold font-aeoniktrial_regular text-[8px] md:text-[12px] bg-black py-2 px-2 flex-initial w-24 text-center text-white rounded-[5px] relative top-[18px] z-10"
      ...
      <div class="addProduct text-center flex justify-center h-0">
        <div class="flex  flex-row gap-2">
          <a href="{{ route('producto', $item->id) }}"
            class="font-semibold font-aeoniktrial_regular text-[16px]  bg-[#FF3D02] py-2 px-4 text-center text-white rounded-xl h-10">
            Ver producto
          </a>
          <button href="{{ route('producto', $item->id) }}" type="button" id='btnAgregarCarrito'
            title="Agregar al Carrito" data-id="{{ $item->id }}"
            class="flex items-center font-aeoniktrial_regular tippy font-semibold text-[13px] bg-[#FF3D02]  py-1 px-4 text-center text-white rounded-xl h-10">
  ...
  <a href="{{ route('producto', $item->id) }}">
    <h2 id="h2Container"
      class="text-base  text-left line-clamp-2  text-ellipsis font-bold font-aeoniktrial_regular tracking-tight  cortartexto tippy max-h-12 md:min-h-12 mt-3 md:mt-0"
      title="{{ $item->producto }}">
    ...
    <div class="flex content-between flex-row gap-4 items-center justify-start !font-Helvetica_Medium pb-4">
      @if ($item->descuento == 0)
        <span class="text-[#FF3D02] text-lg font-bold font-aeoniktrial_regular"> S/. {{ $item->precio }}</span>
      @else
        <div class="flex flex-row gap-2 items-center">
          <span class="text-[#FF3D02] text-lg font-bold font-aeoniktrial_regular">S/. {{ $item->descuento }}</span>
