      ...
      <div class="flex flex-col gap-5">
        <div class="rounded-full w-24 h-24 bg-[#E9EDEF] flex justify-center items-center relative">
          ...
          <label for="upload_image"
            class="bg-[#FF3D02] rounded-full w-7 h-7 flex justify-center items-center absolute bottom-0 right-0 cursor-pointer">
        ...
        <div>
          <p class="font-semibold text-[14px] text-[#001429]">
            {{ $user->name }}
          </p>

          <p class="font-medium text-[12px] text-[#001429]">
            {{ $user->email }}
          </p>
        </div>
      </div>
      <div class="flex flex-col gap-4 ">
        ...
        <form method="POST" action="{{ route('logout') }}" x-data class="group">
          ...
          <button type="submit" href="{{ route('logout') }}"
            class="rounded-2xl bg-[#F3F5F7] md:bg-[#FCFCFC] group-hover:bg-[#FF3D02]  group-hover:text-white text-[#151515] font-medium text-[16px] py-3 px-4 flex justify-between items-center w-64 mt-0 md:mt-[200px]">
