    ...
    >
        ...
        <div className="absolute top-2 left-2 w-max">
          {item.tags?.map((tag) => (
            <div className="px-4 mb-1" key={tag.id}>
              <span
                className="block font-semibold font-aeoniktrial_light text-[8px] md:text-[12px] bg-black py-2 px-3 flex-initial w-full text-center text-white rounded-[5px] relative top-[18px] z-10"
          ...
          <div className="addProduct text-center flex justify-center h-0">
            <div className='flex flex-row gap-2 items-center'>
              <a
                ...
                className="font-semibold  font-aeoniktrial_regular text-[16px] bg-[#ff560a] py-2 px-4 text-center text-white rounded-xl h-10"
              >
                Ver producto
              </a>
              <Tippy content="Agregar al Carrito">
                <button href={`/producto/${item.id}`} type="button" id='btnAgregarCarrito'
                  data-id={`${item.id}`}
                  className="flex items-center font-semibold font-aeoniktrial_regular text-[13px] bg-[#ff560a] py-1 px-4 text-center text-white rounded-xl h-10">
      ...
      <a href={`/producto/${item.id}`} className='p-2'>
        <Tippy content={item.producto}>
          <h2 className="block text-base text-left overflow-hidden font-bold font-aeoniktrial_regular" style={{ display: '-webkit-box', WebkitLineClamp: 2, textOverflow: 'ellipsis', WebkitBoxOrient: 'vertical', height: '51px' }}>
            {item.producto}
          </h2>
        </Tippy>

        {
          is_reseller ?
            (<>
              ...
              <div className="flex content-between flex-row gap-4 items-center justify-start">
                Reseller <span className="text-[#ff560a] text-[16.45px] font-bold font-aeoniktrial_regular">S/. {item.precio_reseller}</span>

              </div></>

            ) :
            (<div className="w-full flex content-between flex-row gap-4 items-center justify-start text-left">
              <span className="text-[#ff560a] font-aeoniktrial_regular font-bold ">{item.descuento > 0 ? 
              ...
              {item.descuento > 0 && (
                <>
                  <div className='flex flex-col items-start'>
                    ...
                    <span className=" text-[#15294C] font-font-aeoniktrial_regular opacity-80 line-through font-bold text-[12px] md:text-base"> S/. {item.precio}</span>
                  </div>
                </>
              )}
            </div>)
